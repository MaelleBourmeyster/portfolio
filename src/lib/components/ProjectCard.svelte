...
>
	<!-- Image Container -->
	<div
		class="relative mb-4 aspect-[4/3] w-full overflow-hidden border-2 border-pk-ink bg-pk-surface"
	>
		{#if thumbnail || image}
			...
			{/if}
		{:else}
			<div class="flex h-full w-full items-center justify-center bg-pk-surface-hover text-pk-muted">
				{$t.project.noImage}
			</div>
		{/if}
	...
	<div class="flex items-start justify-between">
		<div>
			<h3
				class="mb-1 text-xl leading-none font-bold uppercase transition-colors group-hover:text-pk-accent"
			>
				{getStr(title, $language)}
			</h3>
			<p class="font-mono text-sm text-pk-muted">{getStr(category, $language)}</p>
		</div>
		<div
