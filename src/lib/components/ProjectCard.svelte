...
<div class="group relative bg-white border-2 border-black p-4 transition-all hover:-translate-y-2 hover:shadow-[8px_8px_0px_#000] shadow-[4px_4px_0px_#000]">
  ...
  <div class="relative aspect-[4/3] w-full overflow-hidden border-2 border-black bg-gray-100 mb-4">
    {#if image}
      <img src={image} alt={title} class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105 grayscale group-hover:grayscale-0" />
    {:else}
  ...
  <div class="flex items-start justify-between">
    <div>
      <h3 class="text-xl font-bold uppercase leading-none mb-1 group-hover:text-blue-600 transition-colors">{title}</h3>
      <p class="text-sm text-gray-500 font-mono">{category}</p>
    </div>
